...
装 4 个 label：日报（7:05 触发后 `--wait-for-wake` 等 Watch 睡眠同步、每 5 分钟一查，最晚 13:00 强制投递）、频道转发（6–22 每 2 小时）、成长挖掘（9:30 / 15:30 / 21:30）、成长周报（周六 9:45）。**实际调度以 plist 为准**——`config.yaml` 里的 `schedule` 段当前没有代码读取。
