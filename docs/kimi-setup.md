...
本项目的 `LLMClient` (`src/chat_daily_tg/llm_client.py`) 走的是 OpenAI
兼容的 `/v1/chat/completions` 协议, Kimi 官方 API 原生兼容, **无需改
任何代码**, 只要改 `~/chat-daily/config.yaml` 里的 `llm` 段 + 设一个
环境变量即可。
...
## 配置示例 (`~/chat-daily/config.yaml`)

### 本机当前配置：走 Moonshot 官方 API

本机 `/Users/Apple/.hermes/.env` 里已有 `KIMI_API_KEY`，且
`KIMI_BASE_URL=https://api.moonshot.cn/v1`。当前运行配置直接走
Moonshot 官方 OpenAI 兼容接口：
...
如果以后不走本地代理，直接用 Moonshot 官方 key，可以改成：
