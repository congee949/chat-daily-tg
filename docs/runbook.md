...
新建的 `YOUTUBE_API_KEY` 只放行 YouTube Data API v3（补时长/播放量，Shorts 过
滤）。两个 key 各司其职，**不要合并也不要互换**：新 key 调 Gemini 同样 403。
缺失时 enrichment 自动降级 watch-page 抓取（不报错、卡片缺时长），与
`VIBEKEY_API_KEY` 缺失同款静默退化。验证：`curl --proxy http://100.87.113.14:8888
