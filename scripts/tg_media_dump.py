...
Run UNDER the kabi-tg-cli interpreter (it has telethon + the logged-in session):
    <kabi-tg-cli-python> tg_media_dump.py <chat_id> <since> <until> <out_dir> <limit>
...
async def main() -> int:
    ...
    async with tc.connect() as client:
        ...
        count = 0
        # offset_date=end → telethon starts at the window's upper bound and walks older,
        # so `limit` is spent on in-window (and older) messages instead of being burned
        # on TODAY's messages that are newer than the window (which would otherwise
        # exhaust the budget before reaching the target day on high-volume channels).
        async for msg in client.iter_messages(entity, limit=limit, offset_date=end, min_id=min_id):
